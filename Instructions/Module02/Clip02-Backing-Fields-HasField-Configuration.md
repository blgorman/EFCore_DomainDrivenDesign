...
- Reloaded order shows correct Lines count from the _lines backing field. The demo saves the
  added line, calls ctx.ChangeTracker.Clear(), and re-queries, so the count on that screen is
  materialized from the database into _lines rather than read from the in-memory instance.
- The demo deletes the line it added as its last action, so the database is left as it was found
  and the demo prints the same counts every time it runs.
...
- Action: Delete lines 42 through 46 and put these two lines in their place:
  ```cs
...
- Action:
  ...
  3. Remove the `//` prefix from lines 31 through 132, the remaining commented-out demo body.
