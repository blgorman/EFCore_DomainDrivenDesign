...
- `Order_Lines_LoadedFromDatabase_ViaHasFieldConfig` — Passes before Clip 2 (EF writes to the public `List<T>` directly). **FAILS** if you complete steps 1–2 but skip step 3 (`HasField`) — `reloaded.Lines.Count` is 0. This is the most common mistake. Passes again after all steps are complete.
