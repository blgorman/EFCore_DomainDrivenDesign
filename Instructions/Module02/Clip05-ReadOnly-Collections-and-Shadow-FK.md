...
## Plumbing Note
- Demo seeds customers via SeedDataHelper.EnsureCustomersAsync, then places and saves its own order.
...
## What To Verify
- Before student changes, the demo shows only a "Not Yet Implemented" box.
- After student changes: order.CustomerId no longer compiles anywhere (it's been removed from Order).
- After Step 11, the demo shows one screen: the customer Id passed to Order.Place, the shadow CustomerId (0) right after Order.Place, and the CustomerId stored in the database after the demo sets the shadow property, saves, and reloads the order in a fresh OrderingContext.
...
## Why the Demos Set CustomerId Through the Change Tracker
After this clip, `Order.Place(customerId, lines)` still receives a customer Id, but `Order` no longer stores the value.
The customer Id is saved only when code sets the shadow property before `SaveChangesAsync`:

```cs
ctx.Entry(order).Property<int>("CustomerId").CurrentValue = customerId;
```

The demos in this clip save orders directly through the `DbContext`, so each demo sets the shadow property with that line.

> **Note:** This line is contrived for demo purposes and is **NOT** production-ready code.

Every demo in this clip hard-codes the customer, such as `SeedDataHelper.CustomerAId`. Nothing in this clip lets a caller choose which customer an order belongs to, so a request such as "add an order for customer 7" has no path to follow. When we implement the repository, this line moves into
`OrderRepository.Add(Order order, int customerId)`, and application code passes the customer Id in by calling
`_repo.Add(order, customerId)` without ever touching the change tracker.

Because this is a learning repository, `IOrderRepository` keeps both `Add(Order order)` and `Add(Order order, int customerId)`, so the demos run from the first module to the last. In a real application, you would remove `Add(Order order)` and accept new orders only through `Add(Order order, int customerId)`, so the shadow `CustomerId` is always set.
...
- Delete: `CustomerId = customerId,` from the object initializer. Add nothing after it.
  The `customerId` parameter is still received by the method, but `Order` does not store the value. Code that saves the order sets the shadow `CustomerId` before `SaveChangesAsync`, as shown in the demo at the end of this clip.
...
### Step 11 — Replace the NTI box and unlock the shadow property demo
- File: `ConsoleAppProject/CodeAndDemonstrations/Module02/DemonstrateReadOnlyCollections.cs`
- Action:
  1. Delete the `//TODO: Module 2 Clip 5 — Delete the "Not Yet Implemented" box below.` comment and the entire box below it, from the `SectionBanner("Shadow Property — Order CustomerId")` line down to and including the `));` that closes it.
  2. Delete the `//TODO: Module 2 Clip 5 — Uncomment ONLY after completing Clip 5 ...` comment.
  3. Remove the `//` prefix from every line of the commented block below it, from `//using var scope = serviceProvider.CreateScope();` through the closing `//));`.
- Leave `await Task.CompletedTask;` at the end of the method unchanged.
- The unlocked block places an order for `SeedDataHelper.CustomerAId`, reads the shadow `CustomerId` right after `Order.Place`, sets the shadow property through `ctx.Entry(order)`, saves, and reloads the order in a fresh `OrderingContext` to read the stored `CustomerId`.
